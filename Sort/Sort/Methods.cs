...
namespace Sort
{
    public static class Methods<T> where T : IComparable
    {
        ...
        public static void sort_fast(T[] items)
        {
            Console.WriteLine("Сортировка бастрая.");
            quicksort(items, 0, items.Length - 1);
        }

        public static void quicksort(T[] items, int left, int right)
        {
            if (left < right)
            {
        ...
        public static int partition(T[] items, int left, int right, int pivotIndex)
        {
